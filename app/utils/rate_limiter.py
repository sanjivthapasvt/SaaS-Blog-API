...
async def get_user_for_rate_limit(request: Request, session: AsyncSession = Depends(get_session)):
    try:
...
async def user_identifier(request: Request, session: AsyncSession = Depends(get_session)):
