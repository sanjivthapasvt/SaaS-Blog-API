...
@router.post(
    "/blogs", 
    dependencies=[Depends(RateLimiter(times=10, minutes=1, identifier=user_identifier))]
)
...
@router.post(
    "/blogs/{blog_id}/like", 
    dependencies=[Depends(RateLimiter(times=20, minutes=1, identifier=user_identifier))]
)
...
@router.get(
    ...
    dependencies=[Depends(RateLimiter(times=60, minutes=1, identifier=user_identifier))]
)
...
@router.get(
    "/blogs/liked", 
    response_model=PaginatedResponse[BlogResponse], 
    dependencies=[Depends(RateLimiter(times=30, minutes=1, identifier=user_identifier))]
)
