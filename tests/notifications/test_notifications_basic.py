...
class TestNotificationBasics:
    ...
    async def test_get_notifications_pagination(self, client: AsyncClient):
        ...
        resp = await client.get("/api/notifications?limit=5&offset=5", headers=user1_headers)
        assert resp.status_code == 200
    ...
    async def test_get_notifications_search(self, client: AsyncClient):
        ...
        for blog in blogs:
            create_resp = await client.post("/api/blogs", data=blog, headers=user1_headers)
            assert create_resp.status_code == 201
        ...
        resp = await client.get("/api/notifications?search=Python", headers=user1_headers)
        assert resp.status_code == 200
        data = resp.json()
