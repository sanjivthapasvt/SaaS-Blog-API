...
class TestUserEndpointsIntegration:
    ...
    async def test_complete_user_workflow(self, client: AsyncClient):
        ...
        followers_resp = await client.get(f"/api/users/{user2_id}/followers", headers=headers1)
        ...
        following_resp = await client.get(f"/api/users/{user1_id}/following", headers=headers1)
        ...
        update_resp = await client.patch(
            "/api/users/me", 
            data={"full_name": "Updated User One"}, 
            headers=headers1
        )
    ...
    async def test_user_search_functionality(self, client: AsyncClient):
        ...
        headers, _ = await self._create_test_user_with_name(client, "Searchable", f"User{uuid4().hex[:8]}")
    ...
    async def _create_test_user_with_name(self, client: AsyncClient, first_name: str, last_name: str) -> tuple[dict[str, str], dict]:
